  ...
  boost::optional<double&> condition_number)
{
    if (pixel_msmts.size() != cameras_.size()) {
        throw std::runtime_error("[CameraSet::triangulateMeasurements] Error: "
                                 "number of measurements does not"
    ...
    // Here we're making an approximation in the case of having a very large
    // number of cameras Select a subset of camera frames from which to
    // triangulate
    ...
    for (int i = 0; i < n_frames; ++i) {
        p_meas.head<2>() =
          cameras_[i * frame_skip].calibrate(pixel_msmts[i * frame_skip]);

        A.block<3, 3>(3 * i, 0) = Eigen::Matrix3d::Identity();
        A.block<3, 1>(3 * i, 3 + i) =
          -(cameras_[i * frame_skip].pose().rotation() * p_meas);
    ...
    if (condition_number) {
        ...
        *condition_number =
        ...
        // result.point = (A.transpose() * A).ldlt().solve(A.transpose() *
        // b).block<3,1>(0,0);
    }
...
TriangulationResult
CameraSet::triangulateMeasurements(
  const aligned_vector<Eigen::Vector2d>& pixel_msmts,
  boost::optional<double&> condition_number)
{
    if (pixel_msmts.size() != cameras_.size()) {
    ...
    Eigen::MatrixXd A =
      Eigen::MatrixXd::Zero(3 * pixel_msmts.size(), pixel_msmts.size() + 3);
...
TriangulationResult
CameraSet::triangulateIterative(
  const aligned_vector<Eigen::Vector2d>& pixel_msmts)
{
    TriangulationResult result;
    ...
    TriangulationResult linear_result =
      triangulateMeasurementsApproximate(pixel_msmts, 10);
    ...
    Vector3dNodePtr point =
      util::allocate_aligned<Vector3dNode>(symbol_shorthand::L(0));
    // point->vector() = cameras_[0].pose().translation();
    // point->vector() +=
    // cameras_[0].pose().transform_from(Eigen::Vector3d(0,0,20));
    ...
    for (size_t i = 0; i < pixel_msmts.size(); ++i) {
        ...
        auto factor = util::allocate_aligned<CeresProjectionFactor>(
          node,
          point,
          pixel_msmts[i],
          px_noise.asDiagonal(),
    ...
    result.max_reprojection_error = max_error;
    result.status =
      succeeded ? TriangulationStatus::SUCCESS : TriangulationStatus::FAILURE;
