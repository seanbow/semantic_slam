...
MLDataAssociator::computeConstraintWeights(const Eigen::MatrixXd& mahals) {
    ...
    munkres_mat.block(0, n, m, m) = params_.mahal_thresh_assign * Eigen::MatrixXd::Ones(m, m);

    // std::cout << "Computing weights with munkres matrix: " << std::endl << munkres_mat << std::endl;

    Eigen::MatrixXi munkres_result = Munkres().solve(munkres_mat);

    Eigen::MatrixXd weights = Eigen::MatrixXd::Zero(m, n + 1);

    // Compute the min of each row in mahals to check for new landmark assignment
    Eigen::VectorXd msmt_mahal_mins;
    if (n > 0) {
        msmt_mahal_mins = mahals.rowwise().minCoeff();
    }

    // TODO check for and remove ambiguous assignments? e.g. if the winning assignment j
    // is such that mahals(i,j) is close to mahals(i,k) for some k != j

    for (int64_t i = 0; i < munkres_result.rows(); ++i) {
        for (int64_t j = 0; j < munkres_result.cols(); ++j) {
            if (munkres_result(i,j) == 0) {
                // Assignment found
                // If j < the number of landmarks, it's an assignment to an existing one
                if (j < n) {
                    weights(i,j) = 1;
                } else {
                    // Assignment returned in padded part of matrix, i.e. "unassigned"
                    // Check if we should initialize a new one
                    if (n == 0 || msmt_mahal_mins(i) > params_.mahal_thresh_init) {
                        weights(i, n) = 1;
                    }
                }
    ...
    // std::cout << "Munkres result is: " << std::endl << munkres_result << std::endl;
