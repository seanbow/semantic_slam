...
#include "semantic_slam/ceres_cost_terms/ceres_camera_constraint.h"

FrontOfCameraConstraint::FrontOfCameraConstraint(
 ...
 */
bool
FrontOfCameraConstraint::Evaluate(double const* const* parameters,
                                  double* residuals,
                                  double** jacobians) const
{
    ...
    if (jacobians) {
        if (jacobians[0])
            memset(jacobians[0], 0, sizeof(double) * 4);
        if (jacobians[1])
            memset(jacobians[1], 0, sizeof(double) * 3);
        if (jacobians[2])
            memset(jacobians[2], 0, sizeof(double) * 3);
    }

    // Check if it is in front of the camera. If not return false i.e.
    // infeasible
    if (camera_p_obj(2) > 0) {
        return true;
    } else {
...
ceres::CostFunction*
FrontOfCameraConstraint::Create(Eigen::Quaterniond body_q_sensor,
                                Eigen::Vector3d body_p_sensor)
{
