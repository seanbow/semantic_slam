...
size_t TwoPointRansac::computeInliers(
    ...
    if (n_points < 5) {
        ...
        // with 2 points exactly it's impossible to do outlier rejection too (2 points will 
        // of course fit the model they define) so reject anything less than 5 or so
...
size_t TwoPointRansac::computeInliersNormalized(
    ...
    for (int i = 0; i < n_hypotheses_; ++i) {
        ...
        //std::cout << "Hypothesis..." << i << "total solutions ..." << hypothesis_storage_.at(i)->totalSolutions << "\n";
...
void TwoPointRansac::setCameraCalibration(double fx, 
                                            double fy, 
                                            double s, 
                                            double u0, 
                                            double v0, 
                                            ...
                                            double p1, 
                                            double p2) {
    camera_matrix_ = (cv::Mat_<double>(3,3) << fx,  0, u0, 
                                                0, fy, v0,
                                                0,  0,  1);
...
double TwoPointRansac::computeSampsonError(
    ...
    err /= Ex1(0)*Ex1(0) + Ex1(1)*Ex1(1) + Ex2(0)*Ex2(0) + Ex2(1)*Ex2(1);

    // Eigen::Vector3d Fx1;
    // Eigen::Vector3d Fx2;
    // Fx1 = E*p1;
    // Fx2 = E.transpose()*p2;
    // return (double) (pow(p2.transpose()*Fx1,2))/( (Fx1(0)*Fx1(0)) + ( Fx1(1)*Fx1(1) ) + (Fx2(1) * Fx2(1) ) + (Fx2(0)*Fx2(0)));

    return err;
}
                 
void TwoPointRansac::solveTwoPoint(
                const Eigen::MatrixXd& points_A,
                const Eigen::MatrixXd& points_B,
                const Eigen::Matrix3d& R,
                int selection)  {
    ...
    // std::cout << "Translation soln = " << translation.transpose() << std::endl;

    hypotheses_[selection]->solution = translation;
}


void TwoPointRansac::selectRandomSet(int num_points, int hyp_index) {
