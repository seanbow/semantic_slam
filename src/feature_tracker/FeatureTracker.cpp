...
void
FeatureTracker::addImage(Frame&& new_frame)
{
...
bool
FeatureTracker::addKeyframeTime(
  ros::Time t,
...
FeatureTracker::extractKeypointsDescriptors(Frame& frame)
{
    ...
    // ROS_INFO_STREAM("Extracted " << frame.keypoints.size() << " ORB
    // keypoints");
}
...
FeatureTracker::trackFeaturesForward(int idx1)
{
    ...
    if (idx1 >= image_buffer_.size() ||
        image_buffer_[idx1].feature_tracks.size() == 0) {
        ROS_WARN_STREAM(
          "Requested feature tracking with no active feature tracks");
        return;
    }
    ...
    cv::Mat descriptors1(frame1.feature_tracks.size(),
    ...
    // ROS_INFO_STREAM("RANSAC: From " << pts1.size() << " features to " <<
    // frame2.feature_tracks.size() << " inliers.");
    ...
    for (size_t i = 0; i < frame2.feature_tracks.size(); ++i) {
        ...
        if (frame2.feature_tracks[i].n_images_in < 5)
            continue;
...
void
FeatureTracker::addNewKeyframeFeatures(Frame& frame)
{
    ...
    std::sort(idx.begin(), idx.end(), [&](size_t i1, size_t i2) {
        return frame.keypoints[i1].response > frame.keypoints[i2].response;
    });
    ...
    // ROS_INFO_STREAM("New keyframe features, index " << index << ", kps size =
    // " << kps.size()
    ...
    for (int i = 0; i < frame.keypoints.size(); ++i) {
        ...
        // Don't need to do this as it will already be in feature_tracks and
        // accounted for in the above loop!! for (int j = 0; j < i; ++j) {
        //     if (good_detection[idx[j]] &&
        //             cv::norm(frame.keypoints[idx[i]].pt -
        //             frame.keypoints[idx[j]].pt) < params_.feature_spacing) {
        //         good_detection[idx[i]] = 0;
        //         break;
        //     }
        // }
...
//         ROS_ERROR_STREAM("[FeatureTracker] Error: dropped IMU message.
//         Expected " << last_imu_seq_ + 1 << ", got " << msg->header.seq);
//     }
...
//         ROS_ERROR_STREAM("[Tracker] dropped image message, non-sequential
//         sequence numbers, received " << msg->header.seq << ", expected " <<
//         last_img_seq_ + 1);
//     }
...
//     // ROS_INFO_STREAM("Integrating from " << last_integrated_imu_time_ << "
//     to " << frame_time);
...
//             q = dq * q;
//         } else {
//             q.coeffs() = (Eigen::Matrix4d::Identity() + imu_dt_/2 *
//             math::quat_Omega(omega)) * q.coeffs();
//         }
...
//     if (t != frame_time) {
//         ROS_WARN_STREAM("Timing off in feature tracker IMU integration;
//         integrated to "  << t << " but requested " << frame_time);
//     }
...
//     // return I_T_C_.rotation().transpose() * rot_int.deltaRij().matrix() *
//     I_T_C_.rotation().matrix();
//     // return I_T_C_.rotation().transpose() * R * I_T_C_.rotation().matrix();
//     return (I_q_C_.conjugate() * q * I_q_C_).toRotationMatrix();
// }

// void FeatureTracker::gyroBiasCallback(const
...
void
FeatureTracker::setCameraExtrinsics(std::vector<double> I_p_C,
...
//     // if using 2pt ransac, to continue, require IMU messages >= last image
//     time if (params_.use_2pt_ransac && last_imu_time_ <
//     img_queue_.front()->header.stamp) return;

//     // compute between frame IMU rotation
//     Eigen::Matrix3d R;
//     if (params_.use_2pt_ransac) R =
//     computeFrameRotation(img_queue_.front()->header.stamp);
//     // Eigen::Matrix3d I2_R_I1 = quat2rot(I2_q_I1);

//     // ROS_INFO_STREAM("Rotation amount = " << 180*2*std::acos(q(3))/M_PI <<
//     " degrees.");
...
//     // if (is_keyframe) {
//     //     ROS_INFO_STREAM("::KLT:: dR between last keyframe times " <<
//     last_keyframe_time_ << " and " << msg->header.stamp << ": ");
//     //     // std::cout << keyframe_dR_ << std::endl;
//     //     Eigen::Quaterniond dqq(keyframe_dR_);
//     //     std::cout << "[ " << dqq.x() << " " << dqq.y() << " " << dqq.z()
//     << " " << dqq.w() << " ]" << std::endl;
//     //     keyframe_dR_ = Eigen::Matrix3d::Identity();
//     // }
...
//     /*
//     long n_ims_sum = 0;
//     for (auto it = tracked_features_.begin(); it != tracked_features_.end();
//     ++it) {
//         n_ims_sum += it->n_images_in;
//     }
//     ROS_INFO_STREAM("Average track length = " << (double)n_ims_sum /
//     tracked_features_.size() << " images.");
//     */
...
//     // forward the original message too to make it easier to process only
//     keyframe images
...
// void FeatureTracker::camInfoCallback(const sensor_msgs::CameraInfo::ConstPtr&
// msg) {
//     // Copy P into K
...
void
FeatureTracker::setCameraCalibration(double fx,
                                     double fy,
                                     double s,
                                     double u0,
                                     double v0,
