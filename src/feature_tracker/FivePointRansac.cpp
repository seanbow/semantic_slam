...
size_t FivePointRansac::computeInliers(
    ...
    if (n_points < 8) {
        ...
        // with 5 points exactly it's impossible to do outlier rejection too (5 points will 
        // of course fit the model they define) so reject anything less than 6-8 or so
...
size_t FivePointRansac::computeInliersNormalized(
...
void FivePointRansac::setCameraCalibration(double fx, 
                                            double fy, 
                                            double s, 
                                            double u0, 
                                            double v0, 
...
double FivePointRansac::computeSampsonError(
    ...
    return (double) (pow(p2.transpose()*essentialMatrix*p1,2))/( (Fx1(0)*Fx1(0)) + ( Fx1(1)*Fx1(1) ) + (Fx2(1) * Fx2(1) ) + (Fx2(0)*Fx2(0)));
}
                 
void FivePointRansac::solveFivePoint(
                const Eigen::MatrixXd &points_A,
                const Eigen::MatrixXd &points_B,
                int selection)  {
    fivePoint(points_A,
...
void FivePointRansac::selectRandomSet(int num_points, int hyp_index) {
    for (int i = 0; i < 5; ++i) {
        bool is_unique;
        do {
            ...
            is_unique = true;
            for (int j = 0; j < i; ++j) {
                if (hypotheses_[hyp_index]->set(j) == hypotheses_[hyp_index]->set(i)) {
                    is_unique = false;
                    break;
                }
            }
