...
ExternalOdometryHandler::setup()
{
    ...
    subscriber_ = nh_.subscribe(
      odometry_topic, 10000, &ExternalOdometryHandler::msgCallback, this);
...
void
ExternalOdometryHandler::msgCallback(const nav_msgs::Odometry::ConstPtr& msg)
{
    received_msgs_++;

    if (received_msgs_ > 1 && msg->header.seq != last_msg_seq_ + 1) {
        ROS_ERROR_STREAM(
          "[Drone Odometry] Error: dropped relative pose message. Expected "
...
Pose3
ExternalOdometryHandler::msgToPose3(const nav_msgs::Odometry& msg) const
{
...
Eigen::MatrixXd
ExternalOdometryHandler::extractOdometryCovariance(
...
// bool ExternalOdometryHandler::getRelativePoseJacobianEstimate(ros::Time t1,
// ros::Time t2, Eigen::MatrixXd& H12)
// {
//     // Assume here that t1 is not too far ahead of nodes that are already in
//     the graph, so:
//     // auto node1 =
//     boost::static_pointer_cast<SE3Node>(graph_->findNearestNode(node_chr_,
//     t1)); auto kf1 = findNearestKeyframe(t1);

//     if (!kf1) return false;

//     Eigen::MatrixXd odom_cov1 = kf1->odometry_covariance();

//     // And assume now (TODO) that t2 IS too far ahead of nodes so we just
//     have to look in the
...
//     // TODO check that the time is close, whether we should add one before
//     it, etc
...
ExternalOdometryHandler::createKeyframe(ros::Time time)
{
    // Integrate up to time and attach a node corresponding to it

    // check size() < 2 instead of !empty(). if we just checked
    // empty, it may be that we get pop a message just before `time` and the
    // queue becomes empty, yet the next message we receive will be after
    // `time`. can't check for this case unless we have both "straddling"
    ...
    //     node_odom_[origin_symbol.key()] = origin; // TODO is this safe to
    //     assume odom origin = actual origin

    //     last_odom_node = util::allocate_aligned<SE3Node>(origin_symbol,
    //     msg_queue_.front().header.stamp); last_odom_node->pose() = origin;
    ...
    //     // Eigen::MatrixXd anchor_cov =
    //     sigmas.array().pow(2).matrix().asDiagonal();
    //     // auto fac =
    //     util::allocate_aligned<CeresSE3PriorFactor>(last_odom_node, origin,
    //     anchor_cov);
    //     // graph_->addFactor(fac);

    //     ROS_INFO_STREAM("Added first node " <<
    //     DefaultKeyFormatter(origin_symbol)
...

    {
        ...
        while (msg_queue_.size() >= 2 &&
               msg_queue_.front().header.stamp <= time) {
    ...
    SemanticKeyframe::Ptr keyframe = util::allocate_aligned<SemanticKeyframe>(
      keyframe_symbol, msg.header.stamp);
    ...
    // Node that here for the initial estimate, we don't want to use the current
    // odometry estimate. We just want to use the odometry *delta* and update
    // the previous optimized estimate
    ...
    // Eigen::Matrix6d cov;
    double sigma_p = 0.015;
    double sigma_q = 0.0015;
    Eigen::VectorXd sigmas(6);
    sigmas << sigma_q, sigma_q, sigma_q, sigma_p, sigma_p, sigma_p;
    Eigen::MatrixXd cov = sigmas.array().pow(2).matrix().asDiagonal();

    auto fac = util::allocate_aligned<CeresBetweenFactor>(
      last_keyframe->graph_node(), keyframe->graph_node(), relp, cov);
    ...
    // ROS_INFO_STREAM("Added new odometry node " <<
    // DefaultKeyFormatter(symbol));

    // ROS_INFO_STREAM("Added spine node " << DefaultKeyFormatter(symbol) << "
    // at time = "
