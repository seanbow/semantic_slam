  ...
  , pnh_("~")
{
    ...
    det_pub_ =
      pnh_.advertise<darknet_ros_msgs::BoundingBoxes>("tracked_objects", 1);

    std::string image_topic, det_topic;

    pnh_.param("image_topic", image_topic, std::string("camera/rgb/image_raw"));
    pnh_.param(
      "detection_topic", det_topic, std::string("detected_objects_in_image"));
...
void
SimpleObjectTracker::detectionCallback(
  ...
  const darknet_ros_msgs::BoundingBoxes::ConstPtr& msg)
{
    ...
    // Mark which objects were observed in this image so we can delete
    // unobserved ones
    ...
    // Iterate downwards so if we erase index i all indices [0,i) remain the
    // same
...
void
SimpleObjectTracker::visualizeTracking(
  const sensor_msgs::ImageConstPtr& image,
  const darknet_ros_msgs::BoundingBoxes& msg)
{
    ...
    for (auto& obj : tracked_objects_) {
        ...
        std::string label =
          obj.object_name + std::string(": ") + std::to_string(obj.id);
...
int
SimpleObjectTracker::getTrackingIndex(const std::string& name,
                                      const cv::Rect2d& drect)
{
    int best_index = -1;
    double best_match = -1;
    ...
    for (size_t i = 0; i < tracked_objects_.size(); ++i) {
        ...
        double match =
          calculateMatchRate(tracked_objects_[i].bounding_box, drect);
...
double
SimpleObjectTracker::calculateMatchRate(const cv::Rect2d& r1,
                                        const cv::Rect2d& r2)
{
    ...
    double len_diag = sqrt(powf(((ir1.width + ir2.width) >> 1), 2) +
                           powf(((ir1.height + ir2.height) >> 1), 2));
    ...
    /* calculate the match rate. The more overlap, the more matching. Contrary,
     * the more deviation, the less matching*/
