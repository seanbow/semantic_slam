...
void
replace_infinites(MatrixXd &matrix) {
...
void 
minimize_along_direction(MatrixXd &matrix, const bool over_columns) {
...
bool 
Munkres::find_uncovered_in_matrix(const double item, size_t &row, size_t &col) const {
  const size_t rows = matrix.rows(),
            columns = matrix.cols();
...
bool 
Munkres::pair_in_list(const std::pair<size_t,size_t> &needle, const std::list<std::pair<size_t,size_t> > &haystack) {
...
Munkres::step2() {
  ...
  if ( covercount >= std::min( matrix.rows(), matrix.cols() ) ) {
#ifdef DEBUG
...
#ifdef DEBUG
  std::cout << "Munkres matrix has " << covercount << " of " << std::min( matrix.rows(), matrix.cols() ) << " Columns covered:" << std::endl;
  for ( size_t row = 0 ; row < rows ; row++ ) {
    for ( size_t col = 0 ; col < columns ; col++ ) {
      std::cout.width(8);
 ...
 */
Eigen::MatrixXi 
Munkres::solve(const MatrixXd &m) {
  ...
  if ( rows != columns ) {
    ...
    // in the matrix.
    // matrix.resize(size, size, matrix.maxCoeff());
    matrix = resizeMatrix(matrix, size, size, matrix.maxCoeff());
  }


  // STAR == 1 == starred, PRIME == 2 == primed
  // mask_matrix.resize(size, size);
  mask_matrix = resizeMatrix(mask_matrix,size,size);
