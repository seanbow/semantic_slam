...
minimize_along_direction(MatrixXd& matrix, const bool over_columns)
{
    ...
    for (size_t i = 0; i < outer_size; i++) {
        ...
        for (size_t j = 1; j < inner_size && min > 0; j++) {
            min =
              std::min<double>(min, over_columns ? matrix(j, i) : matrix(i, j));
        }
...
bool
Munkres::find_uncovered_in_matrix(const double item,
                                  size_t& row,
                                  size_t& col) const
{
    const size_t rows = matrix.rows(), columns = matrix.cols();
...
bool
Munkres::pair_in_list(const std::pair<size_t, size_t>& needle,
...
{
    const size_t rows = m.rows(), columns = m.cols(),
