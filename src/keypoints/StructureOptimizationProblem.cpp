                                     ...
                                     size_t camera_index) const
{
  ...
  double Pqq[9], Pqp[9], Pql[9], Ppp[9], Ppl[9], Pll[9];
  ...
  // cov.block<3, 3>(0, 6) =
  //   Eigen::Map<Eigen::Matrix<double, 3, 3, Eigen::RowMajor>>(Pql);
  // cov.block<3, 3>(3, 3) =
  //   Eigen::Map<Eigen::Matrix<double, 3, 3, Eigen::RowMajor>>(Ppp);
  // cov.block<3, 3>(3, 6) =
  //   Eigen::Map<Eigen::Matrix<double, 3, 3, Eigen::RowMajor>>(Ppl);
  // cov.block<3, 3>(6, 6) =
  //   Eigen::Map<Eigen::Matrix<double, 3, 3, Eigen::RowMajor>>(Pll);
  ...
  const KeypointMeasurement& kp_msmt)
{
  ...
  ceres_problem_.AddResidualBlock(
    projection_cf, cauchy_loss, cam_pose.rotation_data(),
