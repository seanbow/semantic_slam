...
StructureResult optimizeStructure(const Eigen::MatrixXd& points, const ObjectModelBasis& model,
                                  const Eigen::VectorXd& weights)
{
  ...
  for (size_t i = 0; i < m; ++i)
  {
    ...
    // the true values. the caller is responsible for weighting them low. add
    // priors to all points anyway

    Eigen::Vector3d prior_noise = Eigen::Vector3d::Constant(0.05);

    auto prior_model = gtsam::noiseModel::Diagonal::Sigmas(prior_noise);
    gtsam::PriorFactor<gtsam::Point3> prior(sym::L(i), gtsam::Point3(points.col(i)), prior_model);
  ...
  semslam::StructureFactor sf(sym::O(0), landmark_keys, sym::C(0), model, weights);
...
Eigen::VectorXd sample_stddev(const Eigen::MatrixXd& data)
{
  ...
  //     result(i) = std::sqrt( (centered.row(i) * centered.row(i).transpose()) / data.cols() );
  // }
...
StructureResult optimizeStructureFromProjectionWithDepths(const Eigen::MatrixXd& normalized_coords,
                                                          const Eigen::VectorXd& depths, const ObjectModelBasis& model,
                                                          Eigen::VectorXd weights, const ObjectParams& params,
                                                          bool compute_covariance)
{
  ...
  semslam::StructureProjectionFactor spf(normalized_coords, sym::O(0), landmark_keys, sym::C(0), model, weights,
                                         lambda);
  graph.push_back(spf);
  ...
  // cout << "Optimization result -- \nR = " << endl << result.R  << "\nt = " << result.t.transpose() << endl;
  // if (k > 0) cout << " c = " << result.C.transpose() << endl;
...
StructureResult optimizeStructureFromProjection(const Eigen::MatrixXd& normalized_coords, const ObjectModelBasis& model,
                                                Eigen::VectorXd weights, bool compute_covariance)
{
  size_t m = normalized_coords.cols();
  size_t k = model.pc.rows() / 3;
  ...
  StructureResult cd_result = optimizeStructureFromProjectionCoordinateDescent(normalized_coords, model, weights);
...
StructureResult optimizeStructureCoordinateDescent(const Eigen::MatrixXd& points, const ObjectModelBasis& model,
                                                   const Eigen::VectorXd& weights)
{
  ...
  for (size_t iter = 0; iter < max_iters; ++iter)
  {
    ...
    Eigen::JacobiSVD<Eigen::MatrixXd> svd(StDST, Eigen::ComputeThinU | Eigen::ComputeThinV);
