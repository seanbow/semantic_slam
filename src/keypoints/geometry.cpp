                                                ...
                                                Eigen::VectorXd weights, bool compute_covariance)
{
    ...
    ceres::CostFunction* proj_cost = StructureProjectionCostTerm::Create(normalized_coords, 
                                                                       model,
                                                                       weights,
                                                                       lambda);
    ...
    problem.AddParameterBlock(pose.rotation_data(), 4, new ceres::EigenQuaternionParameterization);
    ...
    prior_noise << 10, 10, 10, 1, 1, 1;  // ordering is q,p so there's basically no prior on q here

    ceres::CostFunction* pose_prior = PosePriorCostTerm::Create(prior, prior_noise.asDiagonal());
    problem.AddResidualBlock(pose_prior, NULL, pose.rotation_data(), pose.translation_data());
    ...
    if (compute_covariance) {
      ...
      if (cov.Compute(blocks, &problem)) {
        ...
        for (int i = 0; i < m; ++i) {
          cov.GetCovarianceBlock(&Z.data()[i], 
                                &Z.data()[i], 
                                &result.Z_covariance.data()[i]);
        }
      } else {
