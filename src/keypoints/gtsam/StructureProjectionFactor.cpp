...
namespace semslam
{
StructureProjectionFactor::StructureProjectionFactor(const Eigen::MatrixXd &normalized_measurements,
...
Eigen::MatrixXd StructureProjectionFactor::structure(const gtsam::Values &values) const
{
  if (k_ == 0)
    return model_.mu;
...
gtsam::Vector StructureProjectionFactor::unweightedError(const gtsam::Values &values,
...
void StructureProjectionFactor::weightError(gtsam::Vector &e) const
{
  for (size_t i = 0; i < m_; ++i)
  {
...
void StructureProjectionFactor::weightJacobians(std::vector<gtsam::Matrix> &vec) const
{
  for (auto &H : vec)
    weightJacobian(H);
}

void StructureProjectionFactor::weightJacobian(gtsam::Matrix &H) const
{
  for (size_t i = 0; i < m_; ++i)
  {
...
gtsam::Vector StructureProjectionFactor::weightedError(const gtsam::Values &values) const
{
...
double StructureProjectionFactor::error(const gtsam::Values &values) const
{
  gtsam::Vector e = weightedError(values);
  return 0.5 * e.transpose() * e;
}

boost::shared_ptr<gtsam::GaussianFactor> StructureProjectionFactor::linearize(const gtsam::Values &values) const
{
...
Eigen::MatrixXd StructureProjectionFactor::Dobject(const gtsam::Values &values) const
{
...
Eigen::VectorXd StructureProjectionFactor::Dlandmark(const gtsam::Values &values, size_t landmark_index) const
{
...
Eigen::MatrixXd StructureProjectionFactor::Dcoefficients(const gtsam::Values &values) const
{
