...
namespace semslam {
StructureFactor::StructureFactor(gtsam::Key object_key,
  ...
  , lambda_(lambda)
{
    ...
    // weights(i) repeated thrice along the diagonal plus sqrt(lambda) for
    // regularizer Because we need to specify it as variances not weights, use
    // inverse Eigen::VectorXd rep_weights(3*m_ + k_); for (size_t i = 0; i <
    // m_; ++i) {
    //     rep_weights.segment<3>(3*i) = Eigen::Vector3d::Constant(1 /
    //     weights(i));
    // }
...
Eigen::MatrixXd
StructureFactor::structure(const gtsam::Values& values) const
{
    if (k_ == 0)
        return model_.mu;
...
gtsam::Vector
StructureFactor::unwhitenedError(
...
void
StructureFactor::whitenError(gtsam::Vector& e) const
{
    for (size_t i = 0; i < m_; ++i) {
...
void
StructureFactor::whitenJacobians(std::vector<gtsam::Matrix>& vec) const
{
    for (auto& H : vec) {
        whitenJacobian(H);
    }
...
void
StructureFactor::whitenJacobian(gtsam::Matrix& H) const
{
    for (size_t i = 0; i < m_; ++i) {
...
gtsam::Vector
StructureFactor::whitenedError(const gtsam::Values& values) const
{
...
double
StructureFactor::error(const gtsam::Values& values) const
{
...
StructureFactor::linearize(const gtsam::Values& values) const
{
...
Eigen::MatrixXd
StructureFactor::Dobject(const gtsam::Values& values) const
{
    ...
    for (size_t i = 0; i < m_; ++i) {
        // Eigen::Vector3d p =
        // values.at<gtsam::Point3>(landmark_keys_[i]).vector();
...
Eigen::MatrixXd
StructureFactor::Dlandmark(const gtsam::Values& values,
...
Eigen::MatrixXd
StructureFactor::Dcoefficients(const gtsam::Values& values) const
{
    ...
    for (size_t i = 0; i < m_; ++i) {
        ...
        // Hc.block(i, 0, 1, k_) = -2.0 * (RBi.transpose() * (p - t -
        // R*model_.mu.col(i) - RBi*c) ).transpose();
