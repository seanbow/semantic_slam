                                  ...
                                  double weight)
{
  ...
  if (!initialized_) {
    ROS_WARN_STREAM("[EstimatedKeypoint] Keypoint uninitialized! Initializing "
  ...
  if (measurements_.size() >= 2) {
    ...
    if (mahal_d > chi2inv95(2)) {
      // ROS_WARN_STREAM(fmt::format("REJECTED measurement of keypoint {}, mahal
      // = {}, score = {}",
      //                             id(), mahal_d, msmt.score));
      return;
    }

    // ROS_INFO_STREAM(fmt::format("Adding measurement of keypoint {} with mahal
    // dist {}, score = {}",
    //                             id(), mahal_d, msmt.score));
  }
