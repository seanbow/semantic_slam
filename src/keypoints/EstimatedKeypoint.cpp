...
EstimatedKeypoint::EstimatedKeypoint(
...
void
EstimatedKeypoint::commitGraphSolution()
{
...
void
EstimatedKeypoint::prepareGraphNode()
{
    graph_node_->vector() = global_position_;
}

void
EstimatedKeypoint::addMeasurement(const KeypointMeasurement& msmt,
                                  double weight)
{
    // if we haven't been initialized yet do so from this first measurement
    if (!initialized_) {
        ROS_WARN_STREAM(
          "[EstimatedKeypoint] Keypoint uninitialized! Initializing "
          "from first added measurement");
        initializeFromMeasurement(msmt);
    }
    ...
    if (measurements_.size() >= 2) {
        ...
        if (mahal_d > chi2inv95(2)) {
            // ROS_WARN_STREAM(fmt::format("REJECTED measurement of keypoint {},
            // mahal = {}, score = {}",
            //                             id(), mahal_d, msmt.score));
            return;
        }

        // ROS_INFO_STREAM(fmt::format("Adding measurement of keypoint {} with
        // mahal dist {}, score = {}",
        //                             id(), mahal_d, msmt.score));
    }
    ...
    Eigen::Vector2d noise_vec =
      Eigen::Vector2d::Constant(msmt.pixel_sigma * msmt.pixel_sigma);
    auto camera_node =
...
double
EstimatedKeypoint::measurementWeightSum() const
{
    double sum = 0.0;
...
void
EstimatedKeypoint::initializeFromMeasurement(const KeypointMeasurement& msmt)
{
...
EstimatedKeypoint::removeFromEstimation()
{
    ...
    if (params_.include_objects_in_graph) {
        ...
        for (auto factor : projection_factors_) {
            semantic_graph_->removeFactor(factor);
        }

        semantic_graph_->removeNode(graph_node_);
    }
...
void
EstimatedKeypoint::initializePosition(const KeypointMeasurement& msmt)
{
    // Initialize a point along the camera ray some distance from the camera
    // with a high covariance along the ray
    ...
    Pose3 G_x_I =
      mapper_->keyframes()[Symbol(msmt.measured_key).index()]->pose();
    ...
    Eigen::Matrix3d P_local = Eigen::Vector3d(sigma_xy, sigma_xy, sigma_z)
                                .array()
                                .pow(2)
                                .matrix()
                                .asDiagonal();
    // Rotate into global frame
    Eigen::Matrix3d P = G_x_C.rotation().toRotationMatrix() * P_local *
                        G_x_C.rotation().toRotationMatrix().transpose();
...
bool
EstimatedKeypoint::triangulate(boost::optional<double&> condition)
{
    if (measurements_.size() < 2)
        return false;

    // ROS_INFO_STREAM("Triangulation of landmark " << id() << ". Pre-triang
    // position = " << position());

    Eigen::MatrixXd A =
      Eigen::MatrixXd::Zero(3 * measurements_.size(), measurements_.size() + 3);
    ...
    for (size_t i = 0; i < measurements_.size(); ++i) {
        ...
        Pose3 G_x_I =
          mapper_->keyframes()[Symbol(measurements_[i].measured_key).index()]
            ->pose();
        ...
        p_meas.head<2>() =
          camera_calibration_->calibrate(measurements_[i].pixel_measurement);
    ...
    // If the condition number is requested we have to do SVD of A anyway so use
    // it to solve the system as well
    if (condition) {
        Eigen::JacobiSVD<Eigen::MatrixXd> svd(
          A, Eigen::ComputeThinU | Eigen::ComputeThinV);
        point = svd.solve(b).block<3, 1>(0, 0);
        *condition = svd.singularValues()(0) /
    ...
    if (is_good) {
        ...
        // estimate covariance
        double msmt_noise_sig2 =
          measurements_[0].pixel_sigma * measurements_[0].pixel_sigma;
        msmt_noise_sig2 /=
          (camera_calibration_->fx() * camera_calibration_->fy());

        global_covariance_ =
          msmt_noise_sig2 * (A.transpose() * A).inverse().block<3, 3>(0, 0);
    }
...
double
EstimatedKeypoint::totalMahalanobisError() const
{
    double distance = 0;
...
size_t
EstimatedKeypoint::nMeasurements() const
{
...
double
EstimatedKeypoint::maxMahalanobisDistance() const
{
    double max_err = 0;
    for (auto msmt : measurements_) {
...
double
EstimatedKeypoint::mahalDistanceThreshold() const
{
...
bool
EstimatedKeypoint::checkSafeToAdd()
{
    // TODO TODO TODO
    return true;

    // ROS_WARN_STREAM("      > Checking if object " << parent_->id() << "
    // keypoint " << id() << " safe to add");
    ...
    if (params_.include_objects_in_graph) {
        ...
        if (!triangulation_succeeded) {
            ROS_WARN_STREAM("-- Rejected landmark "
                            << id() << ", triangulation failed (cond = " << cond
                            << ")");
            return false;
        }
    }
    ...
    if (params_.include_objects_in_graph) {
        if (distance < thresh && cond < 1e4) {
            ROS_INFO_STREAM("-- Accepted landmark " << id()
                                                    << ", mahal = " << distance
                                                    << ", cond = " << cond);
            return true;
        } else {
            ROS_WARN_STREAM("-- Rejected landmark " << id()
                                                    << ", mahal = " << distance
                                                    << ", cond = " << cond);
            return false;
        }
        ...
        if (distance < thresh) {
            ROS_INFO_STREAM(
              fmt::format("-- Accepted landmark {}, mahal {} < {} threshold",
                          id(),
                          distance,
                          thresh));
            return true;
            ...
            ROS_WARN_STREAM(
              fmt::format("-- Rejected landmark {}, mahal {} >= {} threshold",
                          id(),
                          distance,
                          thresh));
            return false;
...
void
EstimatedKeypoint::addToGraphForced()
{
    // add to graph no matter what
    // if the safety checks for the current measurements fail throw them out and
    // add a bare point this should only be called by an object which will then
    // add a constraint tying this to an object structure, otherwise the system
    ...
    // note -- if we only have one measurement it's "unsafe" to add this alone
    // as the triangulation is underconstrained. but if we're adding it with
    // object structure then that is still ok. so mark safe if
...
EstimatedKeypoint::tryAddProjectionFactors()
{
    ...
    for (int i = 0; i < measurements_.size(); ++i) {
        ...
        if (kf->inGraph() && !projection_factors_[i]->active()) {
            ...
            semantic_graph_->addFactor(projection_factors_[i]);
        }
    }
}
...
EstimatedKeypoint::addToGraph()
{
    ...
    if (checkSafeToAdd()) {
        ...
        if (params_.include_objects_in_graph) {
            ...
            semantic_graph_->addNode(graph_node_);
            semantic_graph_->addFactors(projection_factors_);
...
void
EstimatedKeypoint::setConstantInGraph()
{
    graph_->setNodeConstant(graph_node_);
    semantic_graph_->setNodeConstant(graph_node_);
}

void
EstimatedKeypoint::setVariableInGraph()
{
    graph_->setNodeVariable(graph_node_);
    semantic_graph_->setNodeVariable(graph_node_);
}

std::vector<Key>
EstimatedKeypoint::getObservedKeys() const
{
    std::vector<Key> keys;
    for (auto& msmt : measurements_) {
...
double
EstimatedKeypoint::computeMahalanobisDistance(
  const KeypointMeasurement& msmt) const
{
    if (msmt.kp_class_id != classid_) {
        ROS_WARN_STREAM("Mahalanobis distance called for landmark "
                        << id() << "with wrong class");
    ...
    // ROS_WARN_STREAM("Computing mahal for object " << parent_->id() << "; kp "
    // << id());
    ...
    // computeProjectionJacobian(G_T_I.rotation().toRotationMatrix(),
    //                                                           G_T_I.translation(),
    //                                                           I_T_C_.rotation().toRotationMatrix(),
    //                                                           position());
    Eigen::Matrix<double, 2, 9> H =
      computeProjectionJacobian(G_T_I, I_T_C_, position());

    Eigen::Matrix2d R = Eigen::Matrix2d::Zero();
    double px_sigma = msmt.pixel_sigma;
    R(0, 0) = px_sigma * px_sigma /
              (camera_calibration_->fx() * camera_calibration_->fx());
    R(1, 1) = px_sigma * px_sigma /
              (camera_calibration_->fy() * camera_calibration_->fy());

    // std::cout << "R = \n" << R << std::endl;
    // std::cout << "px sigma = " << px_sigma << "; fx = " <<
    // camera_calibration_->fx() << std::endl;

    // Eigen::Matrix2d S = H * Plx * H.transpose() + R;
    // double mahal = residual.transpose() * S.inverse() * residual;

    // Eigen::MatrixXd Plx_full = parent_->getPlx(sym::O(parent_->id()),
    // Symbol(msmt.measured_key));
    Eigen::MatrixXd Plx_full =
      mapper_->getPlx(sym::O(parent_->id()), Symbol(msmt.measured_key));
    ...
    Plx.topLeftCorner<3, 3>() =
      Plx_full.block<3, 3>(3 * our_index, 3 * our_index);
    ...
    double mahal =
      residual.transpose() * (H * Plx * H.transpose() + R).lu().solve(residual);

    return mahal;
}

// double EstimatedKeypoint::computeMeasurementLikelihood(const
// KeypointMeasurement& msmt) const
// {
// 	// ROS_INFO_STREAM("Msmt class = " << msmt.class_measurements[msmt_id]
// << ", kp class = " << kp->classid);
...
//     // gtsam::Pose3 G_x_I =
//     state_estimate.at<gtsam::Pose3>(sym::X(msmt.pose_id)); gtsam::Pose3 G_x_I
//     = graph_->calculateEstimate<gtsam::Pose3>(sym::X(msmt.pose_id));
//     gtsam::Pose3 G_x_C = G_x_I.compose(I_T_C_);
//     Eigen::Vector3d G_l;
//     if (in_graph_){
//         G_l =
//         graph_->calculateEstimate<gtsam::Point3>(sym::L(global_id_)).vector();
//     } else {
...
//     G_x_C.translation().transpose());

//     // make gtsam camera & project landmark's estimated position (note:
//     px_msmt must be normalized!) gtsam::CalibratedCamera camera(G_x_C);
...
//             jointMarginalCovariance(msmt.measured_symbol);

//             Plx.block<3,3>(0,0) = joint_marginals(sym::L(global_id_),
//             sym::L(global_id_)); Plx.block<6,6>(3,3) =
//             joint_marginals(msmt.measured_symbol, msmt.measured_symbol);
//             Plx.block<3,6>(0,3) = joint_marginals(sym::L(global_id_),
//             msmt.measured_symbol); Plx.block<6,3>(3,0) =
//             Plx.block<3,6>(0,3).transpose();
//         } else {
//             Plx.block<3,3>(0,0) = global_covariance_;
//             Plx.block<6,6>(3,3) =
//             graph_->marginalCovariance(msmt.measured_symbol);
//         }
...
//     // msmt noise
//     // px_noise in terms of pixels but here we're in normalized coordinates
//     so divide by focal length Eigen::Matrix2d R = Eigen::Matrix2d::Zero();
//     double px_sigma = msmt.pixel_sigma;
//     R(0,0) = px_sigma * px_sigma / (camera_calibration_->fx() *
//     camera_calibration_->fx()); R(1,1) = px_sigma * px_sigma /
//     (camera_calibration_->fy() * camera_calibration_->fy());
...
//     double likelihood = normalPdf(residual, Eigen::Vector2d::Zero(),
//     S_minus_R + R);

//     // double likelihood = std::exp( -0.5 * residual.transpose() * (S_minus_R
//     + R).inverse() * residual );
//     // likelihood /= std::sqrt( (2 * M_PI * (S_minus_R + R)).determinant() );

//     // std::cout << "Received d = " << msmt.depth << ", expected d = " <<
//     range_hat << std::endl;
...
//     // // std::cout << "Expected [" << zhat.transpose() << ", " << range_hat
//     << "], got [" << msmt.normalized_measurement.transpose() << ", " <<
//     msmt.depth << "]" << std::endl;
