...
void subscriberConnectedCallback(const ros::SingleSubscriberPublisher& ssp) {
	ROS_WARN_STREAM("[KITTI Data] Subscriber " << ssp.getSubscriberName() << " connected to topic " << ssp.getTopic() << ".");
}

void imageSubscriberConnectedCallback(const image_transport::SingleSubscriberPublisher& ssp) {
	ROS_WARN_STREAM("[KITTI Data] Subscriber " << ssp.getSubscriberName() << " connected to topic " << ssp.getTopic() << ".");
}
...
int main(int argc, char *argv[]) {
    ...
    boost::function<void(const image_transport::SingleSubscriberPublisher&)> image_fn = imageSubscriberConnectedCallback;

	image_transport::Publisher cam0_pub = it.advertise("cam0/image_raw", 1000, (image_transport::SubscriberStatusCallback)imageSubscriberConnectedCallback);
	ros::Publisher cam0_info_pub = nh.advertise<sensor_msgs::CameraInfo>("cam0/camera_info", 10);

	image_transport::Publisher cam1_pub = it.advertise("cam1/image_raw", 1000, image_fn, image_fn);
	ros::Publisher cam1_info_pub = nh.advertise<sensor_msgs::CameraInfo>("cam1/camera_info", 10);

	// ros::Publisher car_det_pub = nh.advertise<semslam_msgs::ObjectDetection>("/object_detector/car/detections", 1000, (ros::SubscriberStatusCallback)subscriberConnectedCallback);
	// ros::Publisher window_det_pub = nh.advertise<semslam_msgs::ObjectDetection>("/object_detector/window/detections", 1000);
    ...
    ROS_INFO_STREAM("KITTI cam0 has " << cam0_pub.getNumSubscribers() << " subscribers.");

	ROS_INFO_STREAM("Waiting for " << delay_secs << " seconds before publishing data.");
	...
	// std::string kitti_dir("/home/sean/data/raw_data/2011_09_30/2011_09_30_drive_0020_sync");
	// std::string kitti_dir("/home/sean/data/raw_data/2011_09_26/2011_09_26_drive_0014_sync");
	// std::string kitti_dir("/home/sean/data/kitti/sequences/05");
	// std::string kitti_dir("/media/sean/My Book/ubuntu_home/data/dataset/sequences/05");
	// std::string kitti_dir("/media/sean/My Book/ubuntu_home/data/dataset/sequences/05");
	// std::string kitti_dir("/media/sean/My Book/ubuntu_home/data/dataset/sequences/07");
	...
	while (cam_ts_file.good()) {
		...
		if (ts.length() > 0) {
			ros::Time t_ros = ros::Time::fromBoost(boost::posix_time::time_from_string(ts));
			cam_ts.push_back(t_ros);
		}
		*/
	}

	ROS_INFO_STREAM(cam_ts.size() << " camera msmts");// and " << imu_ts.size() << " imu msmts.");
	...
	if (!cam_calib_file) {
		ROS_ERROR_STREAM("[KITTI Data] Unable to open file " << cam_calib_filename);
		exit(1);
	}
...
*/
	...
	// while (ros::ok() && (cam0_pub.getNumSubscribers() == 0 && cam1_pub.getNumSubscribers() == 0)) {
	// 	ROS_INFO_STREAM("[KITTI_data_node] No subscribers, trying again in 1 second...");
	// 	ros::Duration(1.0).sleep();
	// }

	// Find first index
	while (cam_ts[cam_next] * rate < t_start) cam_next++;
	...
	while (ros::ok() && cam_next < cam_ts.size()) {
		...
		// ROS_INFO_STREAM("Publishing object detections for file " << cam_next << " (t = " << cam_ts[cam_next]*rate << ")");
		...
		if (cam_next > 0 && delay > 0) {
			...
			ros::Duration(cam_ts[cam_next] + t_offset - ros::Time::now().toSec()).sleep();
		}

		// ROS_INFO_STREAM("[Kitti data node] publishing t = " << cam_ts[cam_next]);
