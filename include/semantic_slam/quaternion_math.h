...
quat_Omega(const Eigen::Vector3d& omega)
{
    Eigen::Matrix4d Om;
...
Dquat_mul_dq1(const Quaterniond& qa, const Quaterniond& qb)
{
    ...
    Dq_dq1 << qb.w(), qb.z(), -qb.y(), qb.x(), -qb.z(), qb.w(), qb.x(), qb.y(),
      qb.y(), -qb.x(), qb.w(), qb.z(), -qb.x(), -qb.y(), -qb.z(), qb.w();
    return Dq_dq1;
}
...
Dquat_mul_dq2(const Quaterniond& qa, const Quaterniond& qb)
{
    ...
    Dq_dq2 << qa.w(), -qa.z(), qa.y(), qa.x(), qa.z(), qa.w(), -qa.x(), qa.y(),
      -qa.y(), qa.x(), qa.w(), qa.z(), -qa.x(), -qa.y(), -qa.z(), qa.w();
...
Dpoint_transform_dq(const Quaterniond& q, const Eigen::Vector3d& p)
{
    ...
    D << 2 * (p(1) * q.y() + p(2) * q.z()),
      ...
      2 * (p(0) * q.x() + p(1) * q.y()), 2 * p(1) * q.x() - 2 * p(0) * q.y();
