...
Eigen::Matrix<ValueType, RowsAtCompileTime, ColsAtCompileTime> resizeMatrix(
  Eigen::Matrix<ValueType, RowsAtCompileTime, ColsAtCompileTime>& m, 
  int new_rows, 
  int new_cols, 
  ...
  if (new_rows >= rows && new_cols >= cols) {
    ...
    M_new.block(0,0, new_rows,new_cols) = M_orig.block(0,0,new_rows,new_cols);
  }
