...
public:
  ...
  StructureCostTerm(const ObjectModelBasis& model, const Eigen::VectorXd& weights, double lambda = 1.0);

  static ceres::CostFunction* Create(const ObjectModelBasis& model, const Eigen::VectorXd& weights,
                                     double lambda = 1.0);
...
private:
  ...
  template <typename T>
  Eigen::Matrix<T, 3, Eigen::Dynamic> structure(T const* const* parameters) const;
...
ceres::CostFunction* StructureCostTerm::Create(const ObjectModelBasis& model, const Eigen::VectorXd& weights,
                                               double lambda)
{
  StructureCostTerm* cost_term = new StructureCostTerm(model, weights, lambda);
...
StructureCostTerm::StructureCostTerm(const ObjectModelBasis& model, const Eigen::VectorXd& weights, double lambda)
...
template <typename T>
Eigen::Matrix<T, 3, Eigen::Dynamic> StructureCostTerm::structure(T const* const* parameters) const
{
...
template <typename T>
bool StructureCostTerm::unwhitenedError(T const* const* parameters, T* residuals_ptr) const
{
...
template <typename T>
bool StructureCostTerm::whitenError(T* residuals_ptr) const
{
  Eigen::Map<Eigen::Matrix<T, -1, 1>> residuals(residuals_ptr, dim());
...
template <typename T>
bool StructureCostTerm::operator()(T const* const* parameters, T* residuals) const
{
  // parameters p:
