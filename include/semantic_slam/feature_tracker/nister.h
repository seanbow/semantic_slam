...
inline void
fivePoint(const Eigen::Matrix<double, 3, Eigen::Dynamic>& p1,
          ...
          int* numOfSols)
{
    *numOfSols = 0;
    ...
    Eigen::JacobiSVD<Eigen::Matrix<double, 9, 9>> svd(constraint,
                                                      Eigen::ComputeFullV);
    ...
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
            ...
            // std::cout << "size(E[i][j]) = " << E[i][j].rows() << " x " <<
            // E[i][j].cols() << std::endl;
    ...
    Eigen::Matrix<double, 20, 1> trace =
    ...
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
            Eigen::Matrix<double, 20, 1> LEij = o2(L[i][0], E[0][j]) +
                                                o2(L[i][1], E[1][j]) +
                                                o2(L[i][2], E[2][j]);
            M.row(mrow++) = LEij;
        }
    ...
    Eigen::Matrix<std::complex<double>, 9, 10> Evec =
      nullspaceOfConstraint2 * SOLS;
    ...
    for (int s = 0; s < 10; ++s) {
        ...
        if (is_real) {
            ...
            //         for (int i=0;i<5;i++)   {
            //             std::cout << s << " " <<
            //             p2.col(i).transpose()*E*p1.col(i) <<  "cost\n";
            //         }
        }
    }
}
