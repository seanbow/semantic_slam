    ...
    void SetUp()
    {
        ...
        MultiProjectionFactor::Ptr factor =
          util::allocate_aligned<MultiProjectionFactor>(
            landmark_node, Pose3(), nullptr, reproj_err_threshold);
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            auto fac2 = util::allocate_aligned<CeresProjectionFactor>(
...
TEST(MultiProjectionFactorTests, testOneLandmarkNonIdentityITC)
{
    ...
    MultiProjectionFactor::Ptr factor =
      util::allocate_aligned<MultiProjectionFactor>(
        landmark_node, I_T_C, nullptr, reproj_err_threshold);
...
TEST(MultiProjectionFactorTests, testOneLandmarkVerySimpleWithCalibration)
{
    ...
    MultiProjectionFactor::Ptr factor =
      util::allocate_aligned<MultiProjectionFactor>(
        landmark_node, Pose3(), calib, reproj_err_threshold);
...
TEST(MultiProjectionFactorTests, testUnfrozenCameras)
{
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        EXPECT_NEAR(
          graph.getNode<SE3Node>(sym::X(i))->translation()(1), i - 2, 1e-1);
        EXPECT_NEAR(
